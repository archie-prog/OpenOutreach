...
- **Most behavioural + fingerprint detection vectors are now fixed** (§3). **GPU rendering is now DONE (real hardware).** Remaining in progress (§4): Voyager headers, CDP input leak, distribution refinements. Two declined by the user: withdraw-stale-invites, acceptance-rate.
...
### 4.1 GPU rendering (HIGHEST leverage) — ✅ DONE (real hardware GPU)
~~Browser runs under Xvfb → SwiftShader software WebGL.~~ **RESOLVED:** worker now renders on the host's real `:0` display with the genuine Intel GPU. WebGL = `ANGLE (Intel, Mesa Intel(R) HD Graphics 4000, OpenGL 4.2)`, hardware-probe True. See the §"CLOSED, ACHIEVED" entry below for the exact recipe (`recreate-worker-gpu.sh`) and caveats.
...
- **① GPU rendering — CLOSED, ACHIEVED (real hardware).** Earlier verdict ("not achievable") was WRONG — the user pushed back correctly that the laptop has a real GPU and it was a driver/plumbing issue, not a hardware limit. **Root cause:** Xvfb's GLX path is software (llvmpipe) and ANGLE's Vulkan default falls to SwiftShader on this old GPU. **Fix that works:** stop using headless Xvfb — render on the host's REAL logged-in desktop display (`DISPLAY=:0`), which has the genuine Intel hardware GL stack. Recreated `oo-worker` with `--net=host --security-opt label=disable --device /dev/dri/renderD128 --device /dev/dri/renderD129 -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=:0` (see `handovers/recreate-worker-gpu.sh`), host-side `xhost +local:` to let the container reach :0, and Chromium flags `--use-gl=angle --use-angle=gl --ignore-gpu-blocklist --no-sandbox --disable-gpu-sandbox`. **Verified:** WebGL now reports `ANGLE (Intel, Mesa Intel(R) HD Graphics 4000 (IVB GT2), OpenGL 4.2)` and a hardware-GPU probe returns True — genuine hardware pixels, not SwiftShader/llvmpipe. The WebGL override string was updated to this true Intel value (coherent + honest = a real laptop). **Caveats:** `xhost +local:` resets on graphical logout — added `~/.config/autostart/kit-xhost.desktop` so it re-runs on each login; the box has NO auto-login (gdm shows a login screen on reboot) and setting auto-login needs the sudo password we don't have, so a physical reboot still needs someone to log in once. (commit 3c1d9fc)
